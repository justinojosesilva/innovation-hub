...
import { generateSddAction } from "../actions";
import { SubmitButton } from "@/app/_components/submit-button";

export const dynamic = "force-dynamic";

const MATURE = new Set(["MVP", "PRODUCAO"]);

export default async function SddPage({
  params,
}: {
  ...
  const doc = await getDoc(id, "SDD");
  const mature = MATURE.has(idea.status);
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      ...
      <div className="mt-3 mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">SDD</h1>
          <p className="text-xs text-neutral-400">
            Spec-Driven Development — especificação técnica
            {doc && ` · via ${modelLabel(doc.model)}`}
          </p>
        </div>
        {doc && (
          <div className="flex items-center gap-3">
            <a
              href={`/api/ideas/${id}/sdd`}
              download
...
function Locked() {
  return (
    <div className="rounded-xl border border-dashed border-neutral-300 px-6 py-16 text-center dark:border-neutral-700">
      <p className="text-neutral-500">SDD disponível quando a ideia amadurecer.</p>
      <p className="mt-2 text-sm text-neutral-400">
        A especificação técnica é liberada quando o status chega a <strong>MVP</strong> ou{" "}
...
function Empty({ ideaId }: { ideaId: string }) {
  ...
  return (
    <div className="rounded-xl border border-dashed border-neutral-300 px-6 py-16 text-center dark:border-neutral-700">
      <p className="text-neutral-500">SDD ainda não gerado.</p>
      <form action={generateSddAction} className="mt-4">
        <input type="hidden" name="ideaId" value={ideaId} />
        <SubmitButton
          disabled={!hasKey}
          pendingText="Gerando SDD…"
